...
public class DatabaseTreePanel extends JPanel {
  ...
  private FdPopupFactory fdPopupFactory;

  private JPanel pnlNormalForm;
  ...
  public DatabaseTreePanel(Database db) {
	...
	pnlNormalForm = new JPanel(new BorderLayout());
	...
	fdPopupFactory = new FdPopupFactory(tree);
  ...
  private void updateInspectLabels() {
	...
	String nfTextDb = getNFStringDb(node);
  ...
  private String getNFStringDb(DefaultMutableTreeNode node) {
	String normalForm = "";
	GeneralRelationCheck checker = new GeneralRelationCheck();
